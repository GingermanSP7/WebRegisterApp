...
let esame = sql.query("create table if not exists esame(idAppello bigint unsigned REFERENCES appello(idAppello), matricola varchar(10) not null, maxRisposte int unsigned not null, risposteDate int unsigned not null, maxVotoScritto int unsigned not null, formula varchar(30), orale int unsigned not null, laboratorio int unsigned not null, votoComplessivo int unsigned not null,stato varchar(20),foreign key(matricola) references studente(matricola) on delete cascade,primary key(idAppello,matricola));")
